...
- **TypeScript** (solution-style config with project references): the root `tsconfig.json` is a solution config (`files: []`, `references`) pointing at `tsconfig.app.json` (strict, `types: []`, scopes to `_src/assets/js/**/*.ts` browser code) and `tsconfig.node.json` (`allowJs`, `types: ["node"]`, covers all other `.js`/`.mjs` for Node IntelliSense). A single root `tsconfig.json` + `jsconfig.json` at the same level does not work — the TS language server/TSGo prioritizes `tsconfig.json` and ignores `jsconfig.json`, so JS files lose project context and autocomplete (`microsoft/typescript-go#2437`). Project references are the supported fix. `checkJs` is off for the node project (opt in per-file with `// @ts-check`).
- **Pre-commit hook**: `tsgo --build` (type-check via `@typescript/native-preview`, builds both referenced projects) then `lint-staged` (ESLint + Stylelint + Prettier)
